...
def add_user(role, ID, name, dept_name, extra_field, password):
    conn = connect_mysql()
    cursor = conn.cursor()
    if role == "student":
        cursor.execute("""
            INSERT INTO student (ID, name, dept_name, tot_cred, password)
            VALUES (%s, %s, %s, %s, %s)
        """, (ID, name, dept_name, extra_field or 0, password))
    elif role == "instructor":
        cursor.execute("""
            INSERT INTO instructor (ID, name, dept_name, salary, password)
            VALUES (%s, %s, %s, %s, %s)
        """, (ID, name, dept_name, extra_field or 0, password))
    conn.commit()
    cursor.close()
